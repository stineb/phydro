...
Code for this notebook is [here](https://github.com/stineb/phydro/blob/develop/vignettes/acclimating_vpd_response.Rmd).
...
dat2 = list(vpd = exp(seq(log(100), log(3000), length.out = 100 )),
            psi_soil = c(-0.01, -3)) %>%
  expand.grid() %>% 
...
dat1 = list(vpd = exp(seq(log(100), log(3000), length.out = 100)),
            psi_soil = c(-0.01, -3)) %>%
  expand.grid() %>% 
  mutate(dat = purrr::map2(.x = psi_soil, 
                           .y = vpd, 
